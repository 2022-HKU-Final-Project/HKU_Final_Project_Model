...
class InputExample(object):
    ...
    def __init__(
        self, guid, text_a, text_b=None,salary=None,diploma=None,label=None, x0=None, y0=None, x1=None, y1=None
    ):
        """
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids,salary_id,diploma_id, label_id, bboxes=None):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.salary_id = salary_id
        #self.people_num_id = people_num_id
...
def preprocess_data_multiprocessing(data):
    text_a, text_b,salary,diploma,tokenizer, max_seq_length = data
    ...
    examples['salary'] = salary
    #examples['people_num'] = people_num
    examples['diploma'] = diploma
    return examples
...
def preprocess_batch_for_hf_dataset(dataset, tokenizer, max_seq_length):
    if "text_b" in dataset:
        ...
        examples['salary'] = dataset["salary"]
        #examples['people_num'] = dataset["people_num"]
        examples['diploma'] = dataset["diploma"]
        return examples


def preprocess_data(text_a, text_b,salary,diploma,labels, tokenizer, max_seq_length):
    examples =  tokenizer(
        text=text_a,
        text_pair=text_b,
        truncation=True,
...
def build_classification_dataset(
    ...
    if os.path.exists(cached_features_file) and (
        ...
        if len(data) >= 3:
            # Sentence pair task
            text_a, salary,diploma,labels = data
            text_b = None
            print("a")
        else:
        ...
        if (mode == "train" and args.use_multiprocessing) or (
            ...
            if text_b is not None:
                ...
                data = [
                    (text_a[i : i + chunksize], None,salary,diploma, tokenizer, args.max_seq_length)
                    for i in range(0, len(text_a), chunksize)
                ]
            ...
            # print("examples",[[example[key] for example in examples] for key in examples[0]])
            examples = list()
            for item in tqdm(data, desc='convert example to feature'):
                examples.append(preprocess_data_multiprocessing(item))
            print(len(examples))

            examples = {
                key: torch.cat([torch.tensor(example[key]) for example in examples])
                for key in examples[0]
            }
        else:
            examples = preprocess_data(
                text_a, text_b, salary,diploma,labels, tokenizer, args.max_seq_length
            )
...
def load_hf_dataset(data, tokenizer, args, multi_label):
    ...
    if args.model_type in ["bert", "xlnet", "albert", "layoutlm", "layoutlmv2"]:
        dataset.set_format(
            type="pt",
            columns=["input_ids", "token_type_ids", "attention_mask","salary","diploma", "labels"],
        )
    else:
        dataset.set_format(type="pt", columns=["input_ids", "attention_mask", "salary","diploma","labels"])
...
def convert_example_to_feature(
    ...
    if bboxes:
        return InputFeatures(
            ...
            salary_id = example.salary,
            #people_num_id = example.people_num_id,
            diploma_id = example.diploma,
        ...
        return InputFeatures(
            ...
            segment_ids=segment_ids,
            salary_id = example.salary,
            #people_num_id = example.people_num,
            diploma_id = example.diploma,
            label_id=example.label,
        )
