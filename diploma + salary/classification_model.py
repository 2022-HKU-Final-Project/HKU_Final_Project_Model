    ...
    def train_model(
        ...
        **kwargs,
        ...
        if self.args.use_hf_datasets:
            ...
            train_data.asset = load_hf_dataset(
            ...
            if "text" in train_df.columns and "labels" in train_df.columns:
                if self.args.model_type in ["layoutlm", "layoutlmv2"]:
                    train_examples = [
                        InputExample(i, text, None,salary,diploma, label, x0, y0, x1, y1)
                        for i, (text,salary,diploma,label, x0, y0, x1, y1) in enumerate(
                            zip(
                    ...
                    train_examples = (
                        ...
                        train_df['salary'].tolist(),
                        #train_df['people_num'].tolist(),
                        train_df['diploma'].tolist(),
                ...
                train_examples = (
                    ...
                    train_df.iloc[:, 3].tolist(),
                    train_df.iloc[:, 4].tolist(),
                    #train_df.iloc[:, 5].tolist(),
                )
                    ...
                    eval_examples = [
                        ...
                        for i, (text, salary,diploma,label, x0, y0, x1, y1) in enumerate(
                            zip(
	                           ...
	                           eval_df['salary'].tolist(),
	                           #eval_df['people_num'].tolist(),
	                           eval_df['diploma'].tolist(),
                                eval_df["labels"],
                                eval_df["x0"],
                                eval_df["y0"],
                                eval_df["x1"],
        ...
        if args.sliding_window or self.args.model_type in ["layoutlm", "layoutlmv2"]:
            ...
            if os.path.exists(cached_features_file) and (
                ...
                if self.args.model_type not in ["layoutlm", "layoutlmv2"]:
                    if len(examples) >= 3:
                        examples = [
                            InputExample(i, text_a, None,salary,diploma,label)
                            for i, (text_a, salary,diploma, label) in enumerate(zip(*examples))
                        ]
                    else:
                        examples = [
                            InputExample(i, text_a, None,salary,diploma,label)
            ...
            if not isinstance(eval_examples[0], InputExample):
                if len(eval_examples) == 2:
                    ...
                    eval_examples = [
                        ...
                        for i, (example_a, salary,diploma, label) in enumerate(
                            zip(*eval_examples)
                        )
                    ]
    ...
    def _get_inputs_dict(self, batch, no_hf=False):
        ...
        if isinstance(batch[0], dict) or isinstance(batch[0].data, dict):
            ...
            for key, value in batch[0].items():
        	  	 if key in ['diploma','salary','diploma']:
        	  	 	inputs[key] = value.to(self.device)
        	  	 else:
        	  	 	inputs[key] = value.squeeze(1).to(self.device)
#            inputs = {
