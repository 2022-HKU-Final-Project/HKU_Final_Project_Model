        ...
        **kwargs,
        ...
        if self.args.use_hf_datasets:
            ...
            if "text" in train_df.columns and "labels" in train_df.columns:
                if self.args.model_type in ["layoutlm", "layoutlmv2"]:
                    train_examples = [
                        InputExample(i, text, None,diploma, label, x0, y0, x1, y1)
                        for i, (text,diploma,label, x0, y0, x1, y1) in enumerate(
                            zip(
        ...
        if args.sliding_window or self.args.model_type in ["layoutlm", "layoutlmv2"]:
            ...
            if os.path.exists(cached_features_file) and (
                ...
                if self.args.model_type not in ["layoutlm", "layoutlmv2"]:
                    if len(examples) >= 3:
                        examples = [
                            InputExample(i, text_a, None,diploma,label)
                            for i, (text_a, diploma, label) in enumerate(zip(*examples))
                        ]
                    else:
                        examples = [
                            InputExample(i, text_a, None,diploma,label)
            ...
            if not isinstance(eval_examples[0], InputExample):
                if len(eval_examples) == 2:
                    ...
                    eval_examples = [
                        ...
                        for i, (example_a, diploma, label) in enumerate(
                            zip(*eval_examples)
                        )
                    ]
