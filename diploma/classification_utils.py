...
class InputExample(object):
    ...
    def __init__(
        self, guid, text_a, text_b=None,diploma=None,label=None, x0=None, y0=None, x1=None, y1=None
    ):
        """
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids,diploma_id, label_id, bboxes=None):
...
def preprocess_data_multiprocessing(data):
    text_a, text_b,diploma,tokenizer, max_seq_length = data
...
def preprocess_data(text_a, text_b,diploma,labels, tokenizer, max_seq_length):
    examples =  tokenizer(
        text=text_a,
        text_pair=text_b,
        truncation=True,
    ...
    if os.path.exists(cached_features_file) and (
        ...
        if (mode == "train" and args.use_multiprocessing) or (
            ...
            if text_b is not None:
                ...
                data = [
                    (text_a[i : i + chunksize], None,diploma, tokenizer, args.max_seq_length)
                    for i in range(0, len(text_a), chunksize)
                ]
            ...
            examples = preprocess_data(
                text_a, text_b, diploma,labels, tokenizer, args.max_seq_length
            )
...
def load_hf_dataset(data, tokenizer, args, multi_label):
    ...
    if args.model_type in ["bert", "xlnet", "albert", "layoutlm", "layoutlmv2"]:
        dataset.set_format(
            type="pt",
            columns=["input_ids", "token_type_ids", "attention_mask","diploma", "labels"],
        )
    else:
        dataset.set_format(type="pt", columns=["input_ids", "attention_mask", "diploma","labels"])
